...
from paelladoc.domain.models.project import ProjectMemory # Import ProjectMemory for verification
...
@pytest.mark.asyncio
async def test_create_new_project_asks_for_base_path_and_saves_it(memory_adapter: SQLiteMemoryAdapter):
    """
    ...
    response3 = await core_paella(interaction_language=interaction_lang, action="create_new")
    ...
    response4 = await core_paella(interaction_language=interaction_lang, action="create_new", 
    ...
    response5 = await core_paella(interaction_language=interaction_lang, action="create_new", 
                                  documentation_language=doc_lang, new_project_name=project_name)
    assert response5["status"] == "input_needed", f"Expected input_needed, got {response5.get('status')}"
    assert response5["next_param"] == "base_path", f"Expected next_param base_path, got {response5.get('next_param')}"
    ...
    response6 = await core_paella(interaction_language=interaction_lang, action="create_new", 
                                  documentation_language=doc_lang, new_project_name=project_name,
                                  base_path=base_path_input) # Pass the relative path
    assert response6["status"] == "ok", f"Expected status ok, got {response6.get('status')}: {response6.get('message')}"
    assert response6.get("project_name") == project_name
    
    # Verification: Check if project saved with correct absolute base_path
    saved_memory: Optional[ProjectMemory] = await memory_adapter.load_memory(project_name)
    assert saved_memory is not None, f"Project '{project_name}' not found in DB after creation."
    assert saved_memory.metadata.base_path is not None, "Saved metadata base_path is None."
    ...
    if expected_abs_base_path.exists() and expected_abs_base_path.is_dir():
        try:
            ...
            print(f"Warning: Could not clean up test directory {expected_abs_base_path}: {e}") 